...
function syncTimestamp(value, fallbackDate) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return fallbackDate;
  const hours = String(date.getUTCHours()).padStart(2, "0");
  const minutes = String(date.getUTCMinutes()).padStart(2, "0");
  return `${fallbackDate} · ${hours}:${minutes} UTC`;
}
...
export function createStatusCard(snapshot, { theme: themeName = "dark" } = {}) {
  ...
  const syncedAt = syncTimestamp(model.generatedAt, model.lastBuild);
  ...
  return [
    ...
    `    <circle cx="594" cy="36" r="3" fill="${theme.success}"/>`,
    `    <text x="732" y="40" fill="${theme.textSoft}" font-size="10" text-anchor="end">Synced ${escapeXml(syncedAt)}</text>`,
